...
class DQNLearner2(acme.Learner, tf2_savers.TFSaveable):
  """DQN learner2.
...
class FeedForwardActor2(core.Actor):
  ...
  def _policy(self, observation: types.NestedTensor, mask: types.NestedTensor) -> types.NestedTensor:
    ...
    action = trfl.epsilon_greedy(qs, epsilon=0.05, legal_actions_mask=tf.cast(mask, dtype=tf.float32)).sample()
...
class RecurrentActor2(core.Actor):
  ...
  def _policy(
    ...
    qvals = qvals * tf.cast(mask, dtype=tf.float32)

    # Sample from the policy if it is stochastic.
    action = trfl.epsilon_greedy(qvals, epsilon=0.05).sample()
...
csv_logger_for_agent_dqn = loggers.CSVLogger("/content/gdrive/MyDrive/ResultsDQN", label="agent") # Logging csv file directly to colab
csv_logger_for_env_dqn = loggers.CSVLogger("/content/gdrive/MyDrive/ResultsDQN", label="env")# Logging csv file directly to colab
...
csv_logger_for_agent_r2d2 = loggers.CSVLogger("/content/latest_agent_logger_r2d2", label="agent_logging") # Logging csv file directly to colab
csv_logger_for_env_r2d2 = loggers.CSVLogger("/content/latest_env_logger_r2d2", label="env_logging", time_delta=1.)# Logging csv file directly to colab
